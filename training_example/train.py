...
CONTINUE_TRAIN = True
LAST_MODEL_PATH = "F:\\DRL Project\\GitHub\\final-project-2022\\training_example\\saved_models\\best.dat"
...
def train():
    ...
    if TRAIN:
        ...
        if CONTINUE_TRAIN:
            load_checkpoint(agent, LAST_MODEL_PATH)
    else:
    ...
    for eps in range(hyprm.episodes):
        ...
        for step in range(hyprm.maxlength):
            model_state = torch.tensor(state).float().unsqueeze(0).to(device)
            action = agent.select_action(state=model_state)
            ...
            if TRAIN:
                ...
                if len(agent.memory.memories) > hyprm.batchsize:
                    states, actions, rewards, next_states, dones = agent.memory.sample(hyprm.batchsize)
        ...
        if RESTART_TRAIN and bad_episodes >= RESTART_WAIT:
            print(f'Restarting because of no progress. episode {eps}')
            bad_episodes = 0
            agent = Model(env, hyprm, insize, outsize).to(device)
            load_checkpoint(agent, "F:\\DRL Project\\GitHub\\final-project-2022\\training_example\\saved_models\\best.dat")
    print("")
