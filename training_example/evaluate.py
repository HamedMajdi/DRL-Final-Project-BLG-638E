...
LOAD_CUSTOM_MODEL = "F:\\DRL Project\\GitHub\\final-project-2022\\training_example\\saved_models\\best.dat"
    ...
    while True:
        ...
        if CONTROL_OTHER_AGENTS:
            ...
            for agent_obs in agent_observations:
                ...
                agent_state_tensor = torch.tensor(agent_state).float().unsqueeze(0).to(device) 
                agent_action = opponent_agent.select_action(state=agent_state_tensor)
                agent_action = agent_action.detach().cpu().numpy()[0]
